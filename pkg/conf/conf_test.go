...
// TestParseJsonStressWithSqlContext validates all the fields are correctly mapped from a json file to the StressJsonConf
func TestParseJsonStressWithSqlContext(t *testing.T) {

	jsonText := `{
  "queries": [
    {
      "query": "select * FROM \"samples.dremio.com\".\"SF weather 2018-2019.csv LIMIT 50\"",
      "frequency": 5,
      "sqlContext": "Samples"
    },
    {
      "query": "select * FROM \"samples.dremio.com\".\"SF weather 2018-2019.csv\" where \"DATE\" between ':start' and ':end'",
      "frequency": 5,
      "sqlContext": "Samples2",
      "parameters": {
        "start": ["2018-02-04","2018-02-05"],
...
`
	...
	expectedQueryText := "select * FROM \"samples.dremio.com\".\"SF weather 2018-2019.csv LIMIT 50\""
	if *query.QueryText != expectedQueryText {
		t.Errorf("expected query text of %v but was %v", expectedQueryText, *query.QueryText)
	}

	expectedContext := "Samples"
	if *query.SqlContext != expectedContext {
		t.Errorf("expected context %v but was %v", expectedContext, *query.SqlContext)
	}
	...
	expectedQueryText = "select * FROM \"samples.dremio.com\".\"SF weather 2018-2019.csv\" where \"DATE\" between ':start' and ':end'"
	if *query.QueryText != expectedQueryText {
		t.Errorf("expected a query text of %v but was %v", expectedQueryText, query.QueryText)
	}

	expectedContext = "Samples2"
	if *query.SqlContext != expectedContext {
		t.Errorf("expected context %v but was %v", expectedContext, *query.SqlContext)
	}
